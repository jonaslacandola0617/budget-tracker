...
export default function ExpensesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setExpenses(eData); setBudgets(bData); setLoading(false);
  ...
  return (
    <div className="space-y-5 sm:space-y-8 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setEditExpense(null); setModalOpen(true); }} className="btn-primary flex items-center gap-1.5 text-sm">
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
        ...
        <div className="flex gap-2">
          ...
          <select className="input flex-1 sm:flex-none sm:w-auto" value={budgetFilter} onChange={(e) => setBudgetFilter(e.target.value)}>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            {!search && !categoryFilter && !budgetFilter && (
...
function MobileExpenseCard({ expense, onEdit, onDelete }: {
  expense: Expense; onEdit: (e: Expense) => void; onDelete: (id: string) => void;
}) {
  const color = getCategoryColor(expense.category);
  return (
    <div className="flex items-center gap-3 px-4 py-3.5">
      <div className="w-2 h-2 rounded-full flex-shrink-0 mt-0.5" style={{ backgroundColor: color }} />
      <div className="flex-1 min-w-0" onClick={() => onEdit(expense)}>
        <p className="text-sm font-medium text-text-primary truncate">{expense.title}</p>
        <p className="text-xs text-text-dim mt-0.5">{getCategoryLabel(expense.category)} · {formatDate(expense.date)}</p>
      </div>
      <span className="text-sm font-semibold text-text-primary tabular">{formatCurrency(expense.amount)}</span>
      <div className="flex gap-1 flex-shrink-0">
        <button onClick={() => onEdit(expense)} className="w-8 h-8 flex items-center justify-center rounded-lg text-text-dim hover:bg-bg-hover touch-manipulation">
          <Pencil className="w-3.5 h-3.5" />
        </button>
        <button onClick={() => onDelete(expense.id)} className="w-8 h-8 flex items-center justify-center rounded-lg text-text-dim hover:bg-danger/10 hover:text-danger touch-manipulation">
