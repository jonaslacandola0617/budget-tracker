...
import {
  ArrowDownCircle, ArrowUpCircle, Wallet, Receipt,
...
import {
  AnalyticsSummary, Budget, Expense, ExpenseFormData, BudgetFormData,
...
export default function DashboardPage() {
  ...
  const fetchData = async () => {
    ...
    setAnalytics(aData?.totalAllocated !== undefined ? aData : null);
    ...
    <div className="space-y-5 sm:space-y-8 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs sm:text-sm text-text-secondary mt-0.5">Your financial overview</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setBudgetModal(true)}
            className="btn-ghost flex items-center gap-1.5 text-sm border border-border">
      ...
      {/* Central Balance Card — the star of the show */}
      <div className="card p-6 sm:p-8 relative overflow-hidden"
        style={{ background: "linear-gradient(135deg, #181d27 0%, #1a1f2e 100%)" }}>
        {/* decorative ring */}
        <div className="absolute -right-12 -top-12 w-48 h-48 rounded-full border border-accent/10 pointer-events-none" />
        <div className="absolute -right-6 -top-6 w-32 h-32 rounded-full border border-accent/10 pointer-events-none" />

        <div className="relative">
          <p className="text-xs font-medium text-text-secondary uppercase tracking-widest mb-2">Current Balance</p>
          <p className={`text-4xl sm:text-5xl font-bold tabular ${a.currentBalance >= 0 ? "text-text-primary" : "text-danger"}`}
            style={{ fontFamily: "var(--font-display)" }}>
            {formatCurrency(Math.abs(a.currentBalance))}
            {a.currentBalance < 0 && <span className="text-danger text-2xl ml-1">−</span>}
          </p>
          <div className="flex items-center gap-4 mt-4 flex-wrap">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-success" />
              <span className="text-xs text-text-secondary">
                <span className="text-success font-medium">{formatCurrency(a.totalAllocated)}</span> allocated
              </span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-danger" />
              <span className="text-xs text-text-secondary">
                <span className="text-danger font-medium">{formatCurrency(a.totalSpent)}</span> spent
              </span>
            </div>
            <div className="flex items-center gap-2">
              <TrendingUp className="w-3.5 h-3.5 text-accent" />
              <span className="text-xs text-accent font-medium">{formatPercent(a.savingsRate)} remaining</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
        <StatCard label="Total Allocated" value={formatCurrency(a.totalAllocated)} icon={ArrowDownCircle}
          sub={`${a.budgetCount} allocation${a.budgetCount !== 1 ? "s" : ""}`} variant="success" />
        <StatCard label="Total Spent" value={formatCurrency(a.totalSpent)} icon={ArrowUpCircle}
          sub={`${a.totalExpenses} expense${a.totalExpenses !== 1 ? "s" : ""}`}
          variant={a.totalSpent > a.totalAllocated ? "danger" : "default"} />
        <div className="col-span-2 sm:col-span-1">
          <StatCard label="Savings Rate" value={formatPercent(a.savingsRate)} icon={Wallet}
            sub="of total allocated" variant={a.savingsRate > 20 ? "success" : a.savingsRate > 0 ? "warning" : "danger"} />
        </div>
      </div>
        ...
        <div className="card p-4 sm:p-5 lg:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={a.monthlyTrend}>
              ...
              <Area type="monotone" dataKey="allocated" stroke="#22c55e" strokeWidth={1.5} fill="url(#allocG)" strokeDasharray="4 4" name="Allocated" />
              <Area type="monotone" dataKey="spent" stroke="#ef4444" strokeWidth={2} fill="url(#spentG)" name="Spent" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
                ...
                {a.categoryBreakdown.slice(0, 4).map((c) => (
                  <div key={c.category} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-text-secondary">{getCategoryLabel(c.category)}</span>
                    </div>
          ...
          <>
            <div className="sm:hidden divide-y divide-border/50">
              {a.recentExpenses.map((expense) => (
                <div key={expense.id} className="flex items-center gap-3 px-4 py-3 active:bg-bg-hover touch-manipulation"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-text-dim">{getCategoryLabel(expense.category)} · {formatDate(expense.date)}</p>
                  </div>
                  <span className="text-sm font-semibold text-danger tabular flex-shrink-0">−{formatCurrency(expense.amount)}</span>
                </div>
              ))}
            </div>
      ...
      <BudgetModal open={budgetModal} onClose={() => setBudgetModal(false)} onSave={handleSaveBudget} />
